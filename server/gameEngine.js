...
export function startGuessingPhase(room, io, emitRoomUpdate) {
    ...
    room.hotSeat = {
        ...
        playerShuffles: {}, // per-guesser stable shuffle
    };
    ...
    // Assign each guesser a stable shuffle of the hot seat player's cards
    for (const [id, player] of Object.entries(room.players)) {
        if (id !== hotSeatPlayerId && player.connected && !player.pendingMidGameChoice) {
            room.hotSeat.playerShuffles[id] = shuffle([...hotSeatPlayer.cards]);
        }
    }
...
function startCoopGuessingPhase(room, io, emitRoomUpdate) {
    ...
    // Each player gets a stable shuffle of the OTHER player's cards
    room.hotSeat.playerShuffles[idA] = shuffle([...room.players[idB].cards]);
    room.hotSeat.playerShuffles[idB] = shuffle([...room.players[idA].cards]);
...
function handleGuessingTimeout(room, io, emitRoomUpdate) {
    if (room.mode === 'coop') {
        ...
        for (const player of Object.values(room.players)) {
            if (!player.hasGuessed && player.connected) {
                // Use draft guess (synced from client) if available, otherwise fall back to player's stable shuffle
                const otherId = player.id === firstId ? secondId : firstId;
                const otherPlayer = room.players[otherId];
                const fallback = room.hotSeat.playerShuffles?.[player.id] || otherPlayer.cards;
                player.currentGuess = player.draftGuess || fallback.map((c) => c.id);
                player.hasGuessed = true;
            }
        }
        room.timerEndAt = null;
    ...
    for (const player of Object.values(room.players)) {
        if (!player.hasGuessed && player.connected && player.id !== room.hotSeat.playerId) {
            // Use draft guess (synced from client) if available, otherwise fall back to player's stable shuffle
            const fallback = room.hotSeat.playerShuffles?.[player.id] || hotSeatPlayer.cards;
            player.currentGuess = player.draftGuess || fallback.map((c) => c.id);
            player.hasGuessed = true;
        }
    }
