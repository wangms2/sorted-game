    ...
    return (
        <PageLayout>
            <div className="w-full max-w-lg animate-fade-in">
                ...
                <div className="bg-surface rounded-2xl px-3 py-2 mb-4 flex items-center justify-center gap-x-2 gap-y-1.5 flex-wrap">
                    {scoreEntries.map((entry) => (
                        <div key={entry.id} className="flex items-center gap-1 rounded-lg px-1.5">
                            <span
                                className="text-xs font-medium truncate max-w-[4rem] text-white px-1.5 py-0.5 rounded"
                ...
                {/* Player legend (for spotlight player with allGuesses) */}
                {allGuesses && (
                    <div className="flex flex-wrap justify-center gap-1.5 mb-3">
                        {(room.playerOrder || [])
                            .filter(id => id !== hotSeat.playerId && room.players[id]?.connected)
                            .map(id => (
                                <span
                                    key={id}
                    ...
                    <div className="space-y-3 mb-2">
                        {isCoop ? (
                            ...
                            <div className="bg-surface rounded-2xl px-3 py-2.5">
                                ...
                                <div className="flex items-center justify-center gap-x-2 gap-y-1.5 flex-wrap">
                                    {[...scoreEntries].sort((a, b) => b.totalScore - a.totalScore).map((entry) => (
                                        <div key={entry.id} className="flex items-center gap-1">
                                            <span
                                                className="text-white text-xs font-medium truncate max-w-[4rem] px-1.5 py-0.5 rounded"
                                                style={{ backgroundColor: playerColors[entry.id] || '#888' }}
                                            >
                                                {entry.name}
                                            </span>
