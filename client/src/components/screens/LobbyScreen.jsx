...
export default function LobbyScreen() {
    const { room, isHost, players, playerCount, startGame, updateSettings, leaveRoom, kickPlayer } = useGameState();
    const pending = room?.pendingSettings || { totalRounds: 1, timerSeconds: 60 };
    const [totalRounds, setTotalRounds] = useState(pending.totalRounds);
    const [customRounds, setCustomRounds] = useState('');
    const [showCustom, setShowCustom] = useState(false);
    const [timerSeconds, setTimerSeconds] = useState(pending.timerSeconds);
    const [rulesOpen, setRulesOpen] = useState(false);

    // Host: broadcast settings changes to server
    useEffect(() => {
        if (isHost) {
            updateSettings(totalRounds, timerSeconds);
        }
    }, [isHost, totalRounds, timerSeconds, updateSettings]);
    ...
    return (
        <PageLayout>
            <Card className="w-full max-w-md animate-fade-in">
                ...
                {/* Settings — interactive for host, read-only for others */}
                <div className="space-y-4">
                        ...
                        {isHost ? (
                            ...
                            <div className="flex gap-2">
                                {[1, 2, 3].map((n) => (
                                    <div
                                        key={n}
                                        className={`flex-1 py-2 rounded-xl font-semibold text-center ${pending.totalRounds === n
                                            ? 'bg-amber text-white'
                                            : 'bg-surface text-charcoal/30'
                                            }`}
                                    >
                                        {n}
                                    </div>
                                ))}
                                {pending.totalRounds > 3 && (
                                    <div className="flex-1 py-2 rounded-xl font-semibold text-center bg-amber text-white">
                                        {pending.totalRounds}
                                    </div>
                                )}
                            </div>
                    ...
                    <div>
                        ...
                        <div className="relative px-2">
                            ...
                            <div
                                ...
                                style={{ width: `${((isHost ? timerIndex : TIMER_OPTIONS.findIndex((o) => o.val === pending.timerSeconds)) / (TIMER_OPTIONS.length - 1)) * 100}%` }}
                            />
                            {/* Tick marks + labels */}
                            <div className="relative flex justify-between">
                                {TIMER_OPTIONS.map((opt, i) => {
                                    const activeIndex = isHost ? timerIndex : TIMER_OPTIONS.findIndex((o) => o.val === pending.timerSeconds);
                                    const activeVal = isHost ? timerSeconds : pending.timerSeconds;
                                    return (
                                        <button
                                            key={opt.val}
                                            onClick={isHost ? () => setTimerSeconds(opt.val) : undefined}
                                            className={`flex flex-col items-center ${isHost ? 'cursor-pointer group' : 'cursor-default'}`}
                                            disabled={!isHost}
                                        >
                                            <div
                                                className={`w-5 h-5 rounded-full border-2 transition ${i <= activeIndex
                                            ...
                                            />
                                            <span className={`text-xs mt-1 font-medium transition ${activeVal === opt.val
                                                ? 'text-amber'
                                                : 'text-charcoal/40'
                                                }`}>
                                                {opt.label}
                    ...
                    {playerCount >= 2 && (
                        <p className="text-charcoal/40 text-xs text-center font-medium">
                            ~{estimateMinutes(playerCount, isHost ? totalRounds : pending.totalRounds, isHost ? timerSeconds : pending.timerSeconds)} min
                            {(isHost ? totalRounds : pending.totalRounds) > 1 ? ` · ~${estimateMinutes(playerCount, 1, isHost ? timerSeconds : pending.timerSeconds)} min/round` : ''}
                        </p>
                    )}
                    ...
                    {isHost ? (
                        ...
                        <p className="text-charcoal/50 text-center font-medium py-2">
